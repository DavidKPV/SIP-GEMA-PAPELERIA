		...
		<div class="col-xs-12 col-sm-12 col-md-6">
			...
			<?php
				...
				if(isset($_POST['buscar'])){
				 ...
				 <form method="POST">
				 	<div class="form-group">
						 ...
						 <textarea disabled class="form-control" rows="1" required name="id"><?php echo $contenido['id_producto']; ?> </textarea>
					</div>

					<div class="form-group">
						 <label>Nombre del Producto</label>
						 <textarea class="form-control" rows="1" required name="nombre"><?php echo $contenido['nombre_producto']; ?> </textarea>
					</div>
					
					<div class="form-group">
						 <label>Precio del Producto</label>
						 <input type="number" class="form-control" required name="precio" step="any" min="1" max="10000" value=<?php echo $contenido['precio_producto']; ?> >
					</div>

					<div class="form-group">
						 <label>Cantidad del Producto</label>
						 <input type="number" class="form-control" required name="cantidad" min="1" max="1000000" value=<?php echo $contenido['cantidad_producto']; ?> >
					</div>

					<div class="form-group">
					     <label>Descripción del producto</label>
					     <textarea class="form-control" rows="1" required name="descripcion"><?php echo $contenido['descripcion_producto']; ?> </textarea>
					</div>
			    	...
			    	//$codigo=$datos['codigo']; 
			      	?>
  						<input class="form-check-input" type="radio" name="tipo" value="<?php echo $datos['id_tipoproducto']; ?>"
  							<?php
  								if($contenido['id_tipoproducto']===$datos['id_tipoproducto']){
  									?>
  									checked
  									<?php
  								}
  					...
  					<div class="custom-control custom-checkbox my-1 mr-sm-2">
					    ...
					    <label class="custom-control-label" for="customControlInline">SON CORRECTOS LOS NUEVOS DATOS</label>
					    <br><br>
					    <button class="btn btn-outline-success my-2 my-sm-1" type="submit" name="editar">Editar</button>
					  </center>
					</div>
				</form>
				...
				if(isset($_POST['editar'])){
					...
					$descripcion=normaliza($_POST['descripcion']);
					$tipo=$_POST['tipo'];

					$editar=("UPDATE productos SET nombre_producto='$nombre', precio_producto=$precio, cantidad_producto=$cantidad, descripcion_producto='$descripcion', id_tipoproducto='$tipo' WHERE id_producto='$id_def'");
